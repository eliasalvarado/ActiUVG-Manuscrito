...
en la Universidad con la sociedad, contribuyendo así al desarrollo social, cultural, económico y científico
...
visión y compromiso, indicando lo siguiente: 
\begin{quote}
...
\end{quote}
    ...
    tendrán la capacidad de influir directa o indirectamente en su entorno social 
...
\section{Transformación Digital en la Educación Superior}
    ...
    La era digital se caracteriza por la integración de tecnologías digitales en todos los aspectos de la vida. Donde
    ...
    Las universidades, al igual que varias entidades, han tenido que adaptarse a los cambios tecnológicos y sociales que 
    ...
    evolución ha sido impulsada por la propia necesidad de las universidades de mantenerse relevantes, competitivas e 
    ...
    es una necesidad la cual debe de ser abordada con una visión crítica y bajo las particularidades de cada institución
    \cite{chinkes2019transformacionDigital}.


    \subsection{Aplicación Web}
    Una aplicación Web es un programa informático o software el cual se ejecuta en Internet, sin la necesidad de que 
    ...
    aplicación Web permita \cite{pardo2018comparacion}.
        ...
        Esta arquitectura se basa en la división clara de responsabilidad. Por una parte se tiene el cliente, que es la parte
        de la aplicación con la que un usuario interactúa directamente, y por otra parte se tiene el servidor, que es la parte
        encargada de realizar todas las operaciones lógicas necesarias para hacer funcionar la aplicación.
        En esta arquitectura, el cliente envía solicitudes al servidor, el servidor entonces procesa la solicitud bajo lógica
        de negocio y procesamiento de información y finalmente devuelve una respuesta al cliente
        \cite{pardo2018comparacion,urmeneta2025design}.

\section{\textit{Frontend}}
El término \textit{\gls{frontend}}, hace referencia al cliente, el cual es la interfaz gráfica de usuario (GUI, por sus siglas en inglés),
la cual es la parte visual con la que el usuario interactúa directamente. En pocas palabras, es la parte que permite la
interacción del usuario con la aplicación. Bajo el contexto Web, la GUI vista por el usuario desde su navegador se le
conoce como modelo de objetos del documento (DOM, por sus siglas en inglés) \cite{aws2025frontendBackend}.

\subsection{Arquitecturas}
Para el \textit{Frontend} la arquitectura se centra en el lugar donde se realiza el procesamiento y entrega del contenido.
La arquitectura influirá no solo en la experiencia de usuario, sino también en el rendimiento, seguridad y escalabilidad
\cite{romero2021arquitectura}.

\subsubsection{\textit{Single Page Application} (SPA)}
Una SPA es cuando la aplicación se renderiza y corre completamente en el navegador del cliente. Permitiendo una experiencia
de usuario fluida e interactiva, no requiere recargar para actualizar el contenido, ya que realiza llamadas al \textit{Backend}
de manera asíncrona \cite{universidadElBosque2024arquitectura}.

\subsubsection{\textit{Progressive Web App} (PWA)}
Una PWA continúa con el principio de SPA, añadiendo funcionalidades que permiten simular comportamientos que solamente tiene
una aplicación nativa de escritorio o móvil, haciendo posible ejecutar servicios adicionales aún cuando el ambiente donde se
ejecuta la aplicación es el navegador \cite{universidadElBosque2024arquitectura}.
...
El \gls{renderizado} es el proceso mediante el cual el navegador interpreta y transforma el código (HTML, CSS, JavaScript) 
...
ISR permite la actualización de páginas específicas sin necesidad de compilar todo de nuevo. Combinando los beneficios de SSG con
una actualización más fluida y rápida. Ideal cuando SSG no se da abasto debido a su tiempo de compilación.
...
lo cual permite contenido en tiempo real y personalizado. Ideal para aplicaciones con contenido dinámico y cambio constante.
...
El término \textit{\gls{backend}}, hace referencia al servidor que es el encargado de administrar la funcionalidad general
de la aplicación. Esto incluye la lógica de negocio y la gestión del almacenamiento persistente de datos. Además, el
\textit{Backend} es el responsable de manejar cualquier solicitud proveniente del \textit{Frontend}, donde la procesa y
devuelve una respuesta \cite{aws2025frontendBackend}.

\subsection{Arquitecturas}

\subsubsection{\textit{Model-View-Controller} (MVC)}
...
ventajas, entre ellas la facilidad de mantenimiento, escalabilidad y reutilización. Ya que cada componente se puede gestionar de
manera independiente \cite{RANA2022271}.

\subsubsection{Arquitecturas por capas}
La arquitectura por capas se caracteriza por dividir la aplicación en capas lógicas que tienen funciones únicas. La organización
se basa en jerarquía, estructura y separación de responsabilidades según los componentes técnicos del sistema. Cada capa se comunica
únicamente con una capa adyacente. Esto permite que el flujo se propague de manera ordenada de una capa a otra de manera secuencial.
Busca aislamiento y modularidad, garantizando que las modificaciones de una capa no influyan ni afecten a las demás directamente
\cite{richards2021fundamentalsSoftwareArchitecture}.
Las capas más comunes son:
\begin{itemize}
    \item \textbf{Capa de presentación:} Es la responsable de la interacción directa con el usuario, incluyendo validaciones de entrada,
            presentación de resultados y comunicación con la capa de negocio.
    \item \textbf{Capa de negocio:} Es la responsable de reglas de negocio, lógica en procesamiento y orquestación de servicios.
    \item \textbf{Capa de persistencia:} Es la responsable de la gestión de datos, incluyendo comunicaciones con bases de datos, almacenamiento
            y recuperación de datos.
    \item \textbf{Capa de Base de datos:} Es la responsable de almacenar físicamente los datos.
\end{itemize}
...
AWS es una plataforma de \textit{\gls{cloudcomputing}} más completa y adoptada a nivel mundial. Permite a empresas y desarrolladores
acceder a una amplia gama de servicios de computación. Entre los cuales destacan \cite{aws2026aws}:
\begin{itemize}
    \item Servicios de computación: \textit{Amazon Elastic Compute Cloud} (EC2), AWS Lambda, Amazon Lightsail, entre otros. Que permiten
            ejecutar servidores virtuales, aplicaciones sin servidor, contenedores, etc.
    \item Almacenamiento: \textit{Amazon Simple Storage Service} (S3), \textit{Amazon Elastic Block Store} (EBS), \textit{Amazon Glacier},
            entre otros. Que permiten almacenar cualquier cantidad de datos, archivos u objetos de manera segura y escalable.
    \item Bases de datos: \textit{Amazon Relational Database Service} (RDS), \textit{Amazon DynamoDB}, Amazon Aurora, entre otros. Que permiten
            iniciar, gestionar y escalar bases de datos relacionales o no relacionales de manera sencilla y eficiente.
\end{itemize}

\subsubsection{\textit{Amazon Simple Storage Service} (S3)}
S3 es un servicio de Amazon para almacenamiento de objetos basado en la nube. Proporciona disponibilidad, seguridad y alta escalabilidad.
Permite conexiones con otros servicios de AWS, así como aplicaciones de terceros, lo que facilita la integración de esta tecnología a proyectos
como aplicaciones móviles, aplicaciones web, entre otros. Entre sus principales usos, se encuentra el almacenamiento estático de archivos, 
copias de seguridad, archivos multimedia, entre otros \cite{aws2024s3}.

\subsection{Docker}
Docker es una plataforma de código abierto que permite crear, desplegar y ejecutar aplicaciones dentro de contenedores. Un
\gls{container} es una unidad de software ligera y portátil capaz de empaquetar código, librerías, dependencias y configuraciones
...
\subsubsection{Orquestación con \textit{Docker Compose}}
\textit{Docker Compose} es una herramienta que facilita la configuración, interacción y ejecución de múltiples contenedores
Docker. Permite desplegar aplicaciones complejas con múltiples servicios (como bases de datos, servidores web, etc.) de manera
rápida y eficiente \cite{aws2025docker}.
...
\subsection{Vite}
Vite es una herramienta de construcción \textit{frontend} de código abierto, utiliza módulos ES nativos y permite iniciar
servidores de desarrollo rápidos y eficientes gracias a su carga inicial y su \textit{Hot Module Replacement} 
(HMR, por sus siglas en inglés). Configurado para un soporte nativo para TypeScript y otros lenguajes. Además, es
agnóstico al \textit{framework} o librería utilizada, por lo que soporta React \cite{vite2025introduction}.
...
\textit{DOM} de la página web, lo cual permite manipular elementos HTML \cite{aws2025javascript}.
...
errores en tiempo de desarrollo, también conocido como \textit{\gls{type-safe}} \cite{aws2025javascript}.

\subsection{React}
React es una biblioteca JavaScript de código abierto creada por Facebook (Meta). Desde su lanzamiento en 2013, se ha convertido
...
\begin{itemize}
    ...
    \item Su estructura basada en componentes ayuda a la hora de escalar o darle mantenimiento a la aplicación.
...
\subsection{Axios}
Axios es un Cliente HTTP basado en promesas para el navegador y Node.js. Gracias a que es \gls{isomorfico}, puede ejecutarse tanto
...
Storybook es una herramienta de desarrollo de código abierto para crear, visualizar y probar componentes de interfaz de usuario
de forma aislada. Compatible con React, Vue, Angular, entre otros. Permite la creación de una documentación interactiva de
los componentes a través de \textbf{historias}, que son casos de uso específicos que muestran cómo se comporta el componente
bajo diferentes estados, configuraciones y criterios. La integración de storybook en el proceso de desarrollo ayuda a mejorar
la calidad del código, colaboración y consistencia de la interfaz de usuario \cite{storybook2025docs}.

\subsection{Zod}
Zod es una biblioteca de validación de esquemas centrada en TypeScript. Su función principal es validar datos en tiempo de
ejecución, asegurando que cumplan los requisitos definidos en un esquema. Cuenta con una alta gama de tipos y estructura de datos.
Permite validar formularios, datos recibidos de APIs, entre otros casos de uso \cite{zod2025introduction}.

\subsection{Joi}
Joi es una biblioteca de validación de esquemas principalmente diseñada para Node.js y validaciones del lado del servidor
(\textit{backend} API REST). Permite definir estructuras de datos y esquemas para validar datos de entrada. Con una integración
como \textit{middleware} en Express para validación de peticiones HTTP \cite{joi2025introduction}.

\subsection{\textit{JSON Web Token} (JWT)}
\textit{JSON Web Token} es un estándar abierto (RFC 7519) el cual ayuda a definir de manera compacta y autónoma una forma de
...
sistemas de software, a este conjunto de reglas se le conoce como \textit{\gls{endpoint}}. En pocas palabras, una API es un
...
conjunto de condiciones como interfaz uniforme, operaciones HTTP estándar, ausencia de estado y representaciones intercambiables
...
gracias a su enfoque de almacenamiento compartido mediante almacenamiento centralizado y enlaces simbólicos, el cual permite 
...
\subsection{ISO 9241-11:2018}
La norma ISO 9241-11:2018 define la usabilidad como \textit{\guillemetleft{}la medida en que un producto, sistema o servicio puede ser
utilizado por usuarios específicos para lograr objetivos específicos con efectividad, eficiencia y satisfacción en un contexto de uso
específico\guillemetright{}} \cite{ISO9241-11}. Por lo tanto, para esta norma, la usabilidad tiene tres componentes esenciales que son:
eficacia, eficiencia y satisfacción, las cuales permiten evaluar la usabilidad de un sistema, producto de consumo, servicios, etc.

\subsubsection{\textit{System Usability Scale} (SUS)}
Aliado a la norma ISO 9241-11:2018, el Sistema de Escalas de Usabilidad (SUS, por sus siglas en inglés) es una herramienta de evaluación
que permite cuantificar la usabilidad percibida de un sistema.
Consta de 10 preguntas predefinidas con respuestas en una escala de 5 puntos, donde 1 es "totalmente en desacuerdo" y 5 
es "totalmente de acuerdo". El puntaje final se calcula de la siguiente manera \cite{brooke1996sus}:
\begin{enumerate}
    \item Sumando las respuestas de los enunciados impares y restando 5.
    \item Sumando las respuestas de los enunciados pares y restando ese valor a 25.
    \item Sumando ambos resultados y multiplicando por 2.5 para obtener un puntaje entre 0 y 100.
\end{enumerate}
El puntaje SUS promedio es de 68, un resultado por debajo de este valor indica que el sistema tiene muchas áreas de mejora, mientras que un
resultado por encima de este valor indica que el sistema es bastante usable, aunque siempre con espacio para mejoras. Un puntaje SUS de 80 o
más se considera que el sistema es excelente, con una usabilidad sobresaliente \cite{sauro2011sus}.

\subsection{\textit{Loader.io}}
\textit{Loader.io} es una herramienta para pruebas de carga y rendimiento de aplicaciones web y APIs. Permite simular el tráfico de miles de
conexiones simultáneas para evaluar cómo se comporta una aplicación bajo altos niveles de carga, ayudando a identificar cuellos de botella.
Ofrece una capa gratuita con limitaciones, pero muy generosa. No requiere instalación, ya que se ejecuta en la nube y ofrece resultados
detallados con métricas como tiempos de respuesta, tasas de error, entre otros \cite{loaderio2025docs}.

\subsection{Vitest}
Vitest es un marco de pruebas unitarias para JavaScript y con soporte nativo para TypeScript y ESM (Módulos de JavaScript). Es una alternativa
ideal para proyectos de Vue, Angular y React. Su enfoque se basa en la simplicidad y el rendimiento, por lo que es altamente rápido gracias a su
ejecución de pruebas en paralelo y facilidad de configuración \cite{vitest2026gettingStarted}.

\subsection{OWASP ZAP}
OWASP \textit{Zed Attack Proxy} (ZAP, por sus siglas en inglés) es una herramienta de código abierto para encontrar vulnerabilidades en
aplicaciones web. Actúa como un proxy entre el navegador y la aplicación, permitiendo interceptar, inspeccionar y modificar el tráfico
para lograr pruebas de penetración. Permite realizar escaneos pasivos y activos, así como pruebas manuales y automatizadas. Gracias a su
alta biblioteca de ataques predefinidos, permite identificar vulnerabilidades comunes como inyección SQL, Cross-Site Scripting, configuraciones
incorrectas, entre otros. Esto debido a sus herramientas de rastreo \textit{Spider}, la cual analiza el contenido HTML estático con el objetivo
de descubrir enlaces, y \textit{AJAX Spider} que utiliza navegadores reales con el objetivo de interactuar directamente con la aplicación siendo
capaz de realizar acciones como hacer clic en botones \cite{owasp2026documentation}.
