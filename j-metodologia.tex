...
(larga duración, 7 días), con rotación de \textit{tokens} cuando un usuario renueva su sesión (\textit{refresh token}). Estos \textit{tokens}, generados
con JWT, incluyen información relevante del usuario así como su rol, lo que permite una autorización basada en roles para los distintos \textit{endpoints} 
...
Para la construcción de la interfaz de usuario se optó por utilizar \textbf{Material-UI}, ya que además de que ofrece una amplia variedad de componentes
preconstruidos y personalizables para adaptar el diseño y colores para alinear la identidad visual de la plataforma, tiene integración nativa para TypeScript.

\subsubsection{Enrutamiento}
Para la gestión de rutas y navegación dentro de la aplicación se seleccionó \textbf{TanStack Router}, ya que ofrece enrutamiento tipado, gestión de datos
...
Para el manejo de estados globales de la plataforma se seleccionó \textbf{Zustand}, ya que es ligero y optimizado para el rendimiento, permitiendo
...
\begin{itemize}
    \item \textbf{Types:} Este directorio contiene las definiciones de tipos utilizados en el proyecto.
    \item \textbf{Styles:} Este directorio un único archivo que configura el \textit{theme} de Material-UI para toda la aplicación.
...
\begin{itemize}
    ...
    \item \textbf{Tipografía:} Se definieron las fuentes, tamaños, pesos y estilos.
    \item \textbf{\textit{Breakpoints}:} Se definieron los puntos de quiebre para el diseño \textit{responsive}.
\end{itemize}

\subsubsection{Definición de aliases de rutas}
Con el fin de facilitar la importación de archivos, Vite permite definir aliases de rutas que basado en un prefijo, redirigen a la ruta completa y
real del archivo. Por lo que se definieron los siguientes aliases de rutas:
\begin{itemize}
    \item \textbf{@routers}: Directorio de routers.
    \item \textbf{@pages}: Directorio de páginas.
    \item \textbf{@components}: Directorio de componentes.
    \item \textbf{@services}: Directorio de servicios.
    \item \textbf{@stores}: Directorio de stores.
    \item \textbf{@types}: Directorio de tipos.
    \item \textbf{@styles}: Directorio de estilos.
    \item \textbf{@utils}: Directorio de utilidades.
    \item \textbf{@schemas}: Directorio de esquemas de validación.
    \item \textbf{@context}: Directorio de contextos.
    \item \textbf{@hooks:} Directorio de hooks personalizados.
    \item \textbf{@config:} Directorio de configuraciones.
\end{itemize}
